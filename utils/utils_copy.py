...
def polar_bins(vecs: np.ndarray, ANGLE_BINS=16, DIST_BINS=16):
    ang = (np.degrees(np.arctan2(vecs[:,1], vecs[:,0])) + 360) % 360
    dist = np.linalg.norm(vecs, axis=1)
    a_bin = (ang // (360 / ANGLE_BINS)).astype(int)
    d_bin = np.minimum((dist / (dist.max() + 1e-6) * DIST_BINS).astype(int), DIST_BINS - 1)
    return a_bin, d_bin
...
def graph_descriptor(g: nx.Graph, STRUCTURAL_CLASSES: List[str], EDGE_DIM: int, ANGLE_BINS: int = 16,DIST_BINS: int = 16) -> np.ndarray:
    if g.number_of_nodes() == 0:
        return np.zeros(len(STRUCTURAL_CLASSES) + 5 + EDGE_DIM, np.float32)
    ...
    hist /= hist.sum() + 1e-6
    w3d_list     = [d['w3d']     for _, d in g.nodes(data=True)]
    h3d_list     = [d['h3d']     for _, d in g.nodes(data=True)]
    area3d_list  = [d['area3d']  for _, d in g.nodes(data=True)]
    depth_mean   = [d['depth_mean'] for _, d in g.nodes(data=True)]
    depth_std    = [d['depth_std']  for _, d in g.nodes(data=True)]
    ...
    stats = np.array([
        np.mean(w3d_list),      # mean 3D width
        np.mean(h3d_list),      # mean 3D height
        np.mean(area3d_list),   # mean 3D “面積/體積 proxy”
        np.mean(depth_mean),    # mean depth
        np.mean(depth_std),     # mean depth std
    ], np.float32)


    edge_hist = np.zeros(EDGE_DIM, np.float32)
    if g.number_of_edges():
        vecs = np.array([[e['dx'], e['dy']] for _, _, e in g.edges(data=True)], np.float32)
        a, d_bins = polar_bins(vecs)
        mat = np.zeros((ANGLE_BINS, DIST_BINS), np.float32)
        for ai, di in zip(a, d_bins):
            mat[ai, di] += 1
        edge_hist = mat.flatten() / (mat.sum() + 1e-6)
    return np.concatenate([hist, stats, edge_hist]).astype(np.float32)
